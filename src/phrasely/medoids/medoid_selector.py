...
class MedoidSelector:
    ...
    For larger clusters, approximates by choosing the point nearest to
    ...
    def __init__(
        ...
        return_indices: bool = True,  # default True now for visualization
    ):
        metric = metric.lower()
    ...
    def select(
        ...
        """
        Select medoids per cluster.

        Returns
        -------
        (medoid_indices, medoid_phrases)
        """
        ...
        for lbl in unique_labels:
            ...
            if len(idx) == 0:
                continue
            cluster_X = embeddings[idx]
            local_medoid = self._cluster_medoid_index(cluster_X)
            medoid_indices.append(idx[local_medoid])

        medoid_phrases = [phrases[i] for i in medoid_indices]
        logger.info(
            f"Selected {len(medoid_phrases)} medoids across "
            + f"{len(unique_labels)} clusters."
        )
        return medoid_indices, medoid_phrases
    ...
    def _cluster_medoid_index(self, X: np.ndarray) -> int:
        ...
        # Approximate: nearest to centroid
